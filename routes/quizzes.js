...
module.exports = (db) => {
  ...
  router.post("/",(req, res) =>{
    ...
    for (let i = 0; i < numberOfQuestions; i ++) {
      ...
      let question_id;
      db.query(`INSERT INTO questions (content) VALUES ($1) RETURNING *;`, [req.body.str_question[0]])
        .then(data => {
      ...
      for (let j in req.body.str_answer) {
        if (j === Number(req.body.str_options[0])){
          db.query(`INSERT INTO answers (content, is_correct) VALUES ($1, TRUE);`, [req.body.str_answer[j]])
          .then(data => {
          ...
          db.query(`INSERT INTO answers (content) VALUES ($1);`, [req.body.str_answer[j]])
          .then(data => {
            data.rows[0]
          }).catch((err)=>{
            console.log(err.message);
